...
After downloading or cloning the sources of the `FailureCriteria` project,
...
# Cloning the `master` branch of the `FailureCriteria` project

The `master` branch of the `FailureCriteria` project can be cloned as
follows:
 
~~~~{.bash}
$ git clone https://github.com/thelfer/FailureCriteria
~~~~

# Configuration

The sources are assumed to be in the `FailureCriteria` directory. While
not strictly required, it is convienient to create a `build` directory
(the example assumes it to be at the same level as the `FailureCriteria`
directory):
...
`FailureCriteria` may use more refined variables to define the options
